...
/*
Package aws implements a Consul resources provider for ResourceDiscovery server.

See ResourceTypes variable for the list of supported resource types.

Consul provider is configured through a protobuf based config file
(proto/config.proto). Example config:

	{
		consul_filteredservices {}
	}
*/
...
// DefaultProviderID is the provider id to use for this provider if a provider
// id is not configured explicitly.
const DefaultProviderID = "consul"

// ResourceTypes declares resource types supported by the AWS provider.
var ResourceTypes = struct {
	EC2Instances string
}{
	// TODO(cobrien): Change me
	// "ec2_instances",
}
...
// Provider implements a Consul provider for a ResourceDiscovery server.
...
func initConsulProject(c *configpb.ProviderConfig, l *logger.Logger) (map[string]lister, error) {
...
// New creates a Consul provider for RDS server, based on the provided config.
func New(c *configpb.ProviderConfig, l *logger.Logger) (*Provider, error) {
	resourceLister, err := initConsulProject(c, l)
	if err != nil {
		return nil, err
	}
...
func DefaultProviderConfig(resTypes map[string]string, reEvalSec int) *serverconfigpb.Provider {
	...
	return &serverconfigpb.Provider{
		...
		Config: &serverconfigpb.Provider_ConsulConfig{ConsulConfig: c},
	}
}
